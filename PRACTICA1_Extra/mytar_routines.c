...
/*
Argumento de entrada: stream de un fichero
Salida: cheksum crc-16
*/
/*
unsigned short myfletcher16(FILE *file)
{
	int c;
...
{ 
	...
	for(int i = 0; i < nr_files; ++i){
		...
		fread(&auxC, sizeof(unsigned short), 1, tarFile);
...
int createTar(int nFiles, char *fileNames[], char tarName[])
{
	...
	for(int i = 0; i < nFiles; ++i){
		...
		sizeH += sizeof(unsigned short); //para el crc
	}
	...
	for(int i = 0; i < nFiles; ++i){
		...
		unsigned short crc;
		array[i].size = copynFile(inputFile, tarFile, INT_MAX, &crc); //escribe todo el input en el tar y devuelve los bytes escritos
		if(array[i].size == -1){
			return EXIT_FAILURE;
		}
...
int extractTar(char tarName[])
{
	...
	for(int i = 0; i < nFiles; ++i){
		printf("[%d]: Extrayendo fichero %s, tam %d Bytes, CRC %X ...\n", i, array[i].name, array[i].size, array[i].crc);
		...
		int n = copynFile(tarFile, outPut, array[i].size, &crc);
		if(n != array[i].size){
			return EXIT_FAILURE;
		}
