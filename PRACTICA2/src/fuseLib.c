...
{
    char buffer[BLOCK_SIZE_BYTES]; //buffer donde vamos leyendo los bloques
    int bytes2Read, totalRead = 0; //lo que hemos leido y lo que nos queda
    NodeStruct *node = myFileSystem.nodes[fi->fh];  //fi->fh nos dice el i-nodo asociado al fichero y nons lo guardamos en node
    ...
    if (size > node->fileSize - offset){
        bytes2Read = node->fileSize - offset;
    } 
    else{
        bytes2Read = size;
    }
    int totalToRead = bytes2Read;
    ...
    while(bytes2Read) {
        ...
        if(readBlock(&myFileSystem, currentBlock, &buffer)==-1) { //leemos el bloque actual para guardarlo en buffer
            fprintf(stderr,"Error reading blocks in my_read\n");
            return -EIO;
        }

        for(i = offBlock; (i < BLOCK_SIZE_BYTES) && (totalRead < totalToRead); i++) { //copiamos lo que hemos leido al buffer de salida 
...
static int my_unlink(const char *path)
{
    ...
    int idxNode, idxDir = 0;

    if((idxDir = findFileByName(&myFileSystem, (char *)path + 1)) == -1){ //buscamos si existe el fichero con el id en el path del SF
        fprintf(stderr, "--->>> Ese fichero no existe en el sistema %s\n", path);
        return -EEXIST;
    }

    idxNode = myFileSystem.directory.files[idxDir].nodeIdx;
    resizeNode(idxNode, 0); //truncamos el fichero

    myFileSystem.directory.files[idxDir].freeFile = true; //marcar la entrada del fichero como libre
    myFileSystem.directory.numFiles--; 
    myFileSystem.nodes[idxNode]->freeNode = true; //marcar el nodo como libre
    myFileSystem.numFreeNodes++;
    
    updateDirectory(&myFileSystem); //actualizar directorio en el disco virtual
    updateNode(&myFileSystem, idxNode, myFileSystem.nodes[idxNode]); //actualizar inodo en el disco virtual
    free (myFileSystem.nodes[idxNode]); //liberar memoria del inodo
    myFileSystem.nodes[idxNode] = NULL; //actualizar la tabla de inodos
...
struct fuse_operations myFS_operations = {
    ...
    .read       = my_read,                      // Read data outto a file already opened
