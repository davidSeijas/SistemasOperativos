...
void Autobus_En_Parada(){
	...
	pthread_cond_broadcast(&enParada); //todo el que quiera subir o bajar podrá cuando le llegue el turno (mtx)
	...
	while(esperando_bajar[parada_actual] > 0 || esperando_parada[parada_actual] > 0){
		...
		if(esperando_bajar[parada_actual] > 0 && esperando_parada[parada_actual] == 0)
			pthread_cond_wait(&hanBajado, &mtx); //suelto mtx y dejo que los pasajeros bajen hasta que no quede ninguno (me devuelve el mtx con el signal)

		else if(esperando_bajar[parada_actual] == 0 && esperando_parada[parada_actual] > 0)
			pthread_cond_wait(&hanSubido, &mtx); //suelto mtx y dejo que los pasajeros suban hasta que no quede ninguno (me devuelve el mtx con el signal)
		
		else{
			pthread_cond_wait(&hanBajado, &mtx);
            pthread_cond_wait(&hanSubido, &mtx);
        }
	}
...
void Subir_Autobus(int id_usuario, int origen){ 
	...
	while(estado != EN_PARADA || parada_actual != origen || n_ocupantes >= MAX_USUARIOS){ //si alguien solicita subir cuando el bus está en esa parada puede subir sin problemas (siempre que tenga el mutex)
		pthread_cond_wait(&enParada, &mtx); //hasta que no le dejen subir (señal suben) no podrá subir
	}
...
void Bajar_Autobus(int id_usuario, int destino){
	...
	while(estado != EN_PARADA || parada_actual != destino){ //si alguien solicita bajar cuando el bus está en esa parada puede bajar sin problemas (siempre que tenga el mutex)
		pthread_cond_wait(&enParada, &mtx); //hasta que no le dejen bajar no puede
	}
...
int main(int argc, char *argv[]) {
	...
	pthread_cond_init(&enParada, NULL);
